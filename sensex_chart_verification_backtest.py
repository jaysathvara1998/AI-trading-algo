"""
SENSEX Options Chart Verification Backtest: 31-Aug to 02-Sep-2026
==================================================================
100% Real Dhan 1-Min Exchange Bars | SENSEX Index (SID: 51)
Stack:
...
QTY = 20                # 1 Lot SENSEX = 20 qty
CAPITAL = 50000.0
EXPIRY = "BSE SENSEX WEEKLY"
...
def fetch_sensex_bars(feed: DhanDataFeed, days: int = 14) -> pd.DataFrame:
    """Fetch real SENSEX 1-min historical candles from Dhan."""
    dhan = feed.tradehull_client.Dhan
    from_date = (dt.date.today() - dt.timedelta(days=days + 4)).strftime('%Y-%m-%d')
...
def run():
    print("=" * 140)
    print("  SENSEX OPTIONS CHART VERIFICATION BACKTEST | 31-Aug to 02-Sep-2026 | BSE Weekly Expiry")
    print("  100% Real Dhan BSE SENSEX Exchange 1-Min Data | Pure VPIN Strategy (No EMA/VWAP/RSI)")
    print("  Risk Rules: 20% Initial SL | BE Lock @ +20% | Continuous Trail (Peak-15% above +30%) | EOD: 15:24 IST")
    print("=" * 140)

    feed = DhanDataFeed()
    hist = fetch_sensex_bars(feed, days=14)
    if hist.empty:
        print("Failed to fetch SENSEX bars from Dhan API.")
        return

    print(f"\nLoaded {len(hist)} real SENSEX 1-min exchange bars: "
          f"{hist['timestamp'].min().strftime('%d-%b %H:%M')} -> "
          f"{hist['timestamp'].max().strftime('%d-%b %H:%M')} IST")

    # 1. Warm-up & Train Models on Real SENSEX Historical Bars
    ...
    print(f"Engine Warmup Complete. SENSEX Models Trained: SVM={svm_ok}, XGBoost={xgb_ok}\n")
    ...
    for i, row in backtest_bars.iterrows():
        ...
        # Real Ensemble Decision on SENSEX
        ...
        if pos is not None:
            ...
            # SENSEX ATM option delta ~ 0.55
            curr_prem = max(10.0, pos['entry_prem'] + spot_change * 0.55)
            ...
            if is_sl_hit or is_eod or is_rev:
                ...
                all_trades.append({
                    ...
                    'Contract': f"SENSEX {pos['strike']} {pos['opt_type']}",
        ...
        if (pos is None and in_window and not in_cooldown and
            ...
            atm = round(p / 100) * 100  # SENSEX strike step is 100
            # SENSEX ATM option premium ~ spot * 0.0045 (e.g. 76500 * 0.0045 ~ ₹340)
            entry_prem = round(max(100.0, p * 0.0045), 2)
    ...
    # Print Formatted SENSEX Report
    tdf = pd.DataFrame(all_trades)
    print("=" * 140)
    print(f"  SENSEX OPTIONS BACKTEST REPORT | 31-Aug to 02-Sep-2026 | 1 Lot = 20 Qty")
    print(f"  Model Stack: GARCH(1,1) + SVM-RBF + XGBoost + Macro Key Range Guardrail (NO EMA/VWAP/RSI)")
    print("=" * 140)

    if tdf.empty:
        print("\nNo trades were generated on SENSEX in this period.")
        return
    ...
    for date, group in tdf.groupby('Date', sort=False):
        ...
        print(f"{'=' * 140}")
        print(f"  {'#':<3} {'Contract':<26} {'Entry':>8} {'Exit':>8} {'Spot In':>9} {'Spot Out':>9} "
    ...
    print(f"\n{'=' * 80}")
    print(f"  SENSEX OVERALL SUMMARY | 31-Aug to 02-Sep-2026 | Capital: INR 50,000")
    print(f"{'=' * 80}")
    for k, v in [
