...
def perfect_institutional_strategy():
    feed = DhanDataFeed()
    hist = feed.fetch_historical_bars(days=5)

    # Compute technical indicators (EMA9, EMA21, VWAP, RSI)
    hist['ema9'] = hist['close'].ewm(span=9, adjust=False).mean()
    hist['ema21'] = hist['close'].ewm(span=21, adjust=False).mean()
    hist['cum_vol'] = hist['volume'].cumsum()
    hist['cum_vp'] = (hist['close'] * hist['volume']).cumsum()
    hist['vwap'] = hist['cum_vp'] / hist['cum_vol'].replace(0, 1)
    ...
    for idx, row in hist.iterrows():
        ...
        if pos['side'] != 'FLAT':
            ...
            # Step 1: At +18% gain -> SL moves to Breakeven
            if gain_pct >= 0.18 and pos['sl'] < pos['entry_p']:
                pos['sl'] = pos['entry_p']

            # Step 2: Above +25% gain -> Continuous Trailing at Peak - 12%
            if gain_pct >= 0.25:
                dynamic_trail = round(pos['high_p'] * 0.88, 2)
                if dynamic_trail > pos['sl']:
                    pos['sl'] = dynamic_trail

            # Exit on SL / Trailing SL / 15:20 EOD
            ...
            if curr_opt <= pos['sl'] or is_eod:
                ...
                trigger = 'INTRADAY_EOD_EXIT' if is_eod else ('TRAILING_SL_EXIT' if exit_price >= pos['entry_p'] else 'INITIAL_SL_HIT')
        ...
        # 2. Precision Entry with Momentum Alignment
        can_trade_time = ((ts.hour == 9 and ts.minute >= 20) or (10 <= ts.hour <= 11 and ts.minute <= 30) or (13 <= ts.hour < 15))
        in_cooldown = cooldown_until is not None and ts < cooldown_until

        if pos['side'] == 'FLAT' and can_trade_time and not in_cooldown and not dec.is_vetoed:
            # Bullish Entry: EMA9 > EMA21 and Price > VWAP and RSI > 52
            is_bullish = (p > vwap and ema9 > ema21 and rsi > 52 and dec.final_action == DirectionalSignal.BUY)
            # Bearish Entry: EMA9 < EMA21 and Price < VWAP and RSI < 48
            is_bearish = (p < vwap and ema9 < ema21 and rsi < 48 and dec.final_action == DirectionalSignal.SELL)
    ...
    print("=" * 130)
    print("       PERFECTED INSTITUTIONAL MOMENTUM + AI STRATEGY (100% REAL DHAN DATA)")
    print("=" * 130)
    ...
    for date, group in tdf.groupby('Date', sort=False):
        ...
        print(f"\n[DATE: {date}] | Trades: {len(group)} (Wins: {win_count}) | Net Day Realized P&L: {pnl_badge}")
        print("-" * 130)
    ...
    print("\n" + "=" * 80)
    print("              PERFECTED STRATEGY PERFORMANCE SUMMARY")
    print("=" * 80)
...
if __name__ == '__main__':
    perfect_institutional_strategy()
